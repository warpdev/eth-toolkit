...
export const FunctionSignatureSelector = React.memo(function FunctionSignatureSelector({
      ...
      if (
        ...
        if (calldata) {
          ...
          try {
            ...
            // Save to IndexedDB
            await saveSignatureSelection(functionSelector, selectedSig);
          } catch (error) {
  ...
  return (
    <div className="space-y-2">
      ...
      <Select value={selectedIndex.toString()} onValueChange={handleSignatureChange}>
        <SelectTrigger className="w-full font-mono text-sm">
          <SelectValue placeholder="Select a function signature" />
        </SelectTrigger>
        <SelectContent>
          {decodedResult.possibleSignatures.map((sig: string, index: number) => (
            <SelectItem key={index} value={index.toString()} className="font-mono text-sm">
              {sig}
            </SelectItem>
          ))}
        </SelectContent>
