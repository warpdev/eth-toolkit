...
/**
 * GET /api/signatures/function/[signature]
 *
 * Fetches function signatures from 4bytes.directory with aggressive caching
 *
 * @param signature - The hex signature to look up (with or without 0x prefix)
 * @returns Function signature data with appropriate cache headers
 */
export async function GET(request: NextRequest, { params }: { params: { signature: string } }) {
  try {
    ...
    // Validate signature format (should be 8 hex characters for function signatures)
    if (!/^[a-fA-F0-9]{8}$/.test(cleanSignature)) {
      return new Response(
        JSON.stringify({
    ...
    const response = await fetch(
      `https://www.4byte.directory/api/v1/signatures/?hex_signature=${cleanSignature}`,
      {
        // Cache for 7 days (function signatures rarely change)
    ...
    console.error('Unexpected error in function signature API:', error);
